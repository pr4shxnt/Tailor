...
const Navbar = () => {
    ...
    useEffect(() => {
        setIsCollection(location.pathname.startsWith("/category"));
    }, [location]);
    ...
    const addItemToCart = async (newItem) => {
        try {
            await axios.post(`${import.meta.env.VITE_BACKEND_URL}/cart/add`, newItem, {
                headers: { Authorization: `Bearer ${token}` },
            });
            setCartUpdated(true); // Triggers cart refresh
        } catch (error) {
    ...
    return (
        <div className="fixed flex w-full items-center justify-between md:px-10 px-3 shadow-lg z-[999] bg-white font-medium">
            <img src={assets.logo} className="w-28 py-1" alt="Logo" />
            ...
            <div className="flex items-center gap-6">
                {isUserAuthenticated ? (
                    <div className="group relative">
                        <div className="flex items-center gap-2 py-2 border-black border rounded-full px-3">
                            <img src={assets.profile_icon} alt="Profile" className="w-5 cursor-pointer" />
                            <h1 className="hidden md:block">{userData.name}</h1>
                        </div>
                    ...
                    <Link to="/login">
                        <div className="flex items-center gap-2 py-2 border-black border rounded-full px-3">
                            <img src={assets.profile_icon} alt="Login" className="w-5 cursor-pointer" /> Login
                        </div>
                    </Link>
                )}
            ...
            <div className={`fixed z-[999] top-24 m-3 backdrop-blur-3xl right-0 bottom-0 overflow-hidden rounded-lg transition-all ${visible ? "w-[95vw]" : "w-0"}`}>
                <div className="flex flex-col text-gray-600">
                    ...
                    <NavLink onClick={() => setVisible(false)} className="py-2 pl-6" to="/category/all_collection">COLLECTION</NavLink>
